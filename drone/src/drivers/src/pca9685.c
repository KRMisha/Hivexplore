...
bool pca9685init(int addr, float pwmFreq) {
    ...
    return (prescale >= 0x03) && (prescale <= 0xFF) && i2cdevWriteByte(&deckBus, addr, regPreScale, (uint8_t)prescale) &&
           i2cdevWriteByte(&deckBus, addr, regMode1, mode1val);
}

bool pca9685setDuties(int addr, int chanBegin, int nChan, float const* duties) {
...
bool pca9685setDurations(int addr, int chanBegin, int nChan, uint16_t const* durations) {
    uint8_t data[LED_NBYTES];
    for (int i = 0; i < nChan; ++i) {
...
static void asyncTask(__attribute__((unused)) void* param) {
    while (true) {
        ...
        if (ok == pdTRUE) {
            ...
            pca9685setDurations(reqPop.addr, reqPop.chanBegin, reqPop.nChan, reqPop.durations);
        }
    }
}
...
bool pca9685setDutiesAsync(int addr, int chanBegin, int nChan, float const* duties) {
...
bool pca9685setDurationsAsync(int addr, int chanBegin, int nChan, uint16_t const* durations) {
    struct asyncRequest reqPush = {
        .addr = addr,
        .chanBegin = chanBegin,
        .nChan = nChan,
