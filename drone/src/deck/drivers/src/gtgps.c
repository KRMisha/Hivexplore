...
typedef enum { FixNone = 1, Fix2D = 2, Fix3D = 3 } FixQuality;

typedef enum { NoFix = 1, GPSFix = 2 } FixType;
...
static ParserConfig parsers[] = {{.token = "GPGSA", .parser = gpgsaParser}, {.token = "GPGGA", .parser = gpggaParser}};
...
static void gtgpsInit(DeckInfo* info) {
    ...
    xTaskCreate(gtgpsTask, GTGPS_DECK_TASK_NAME, configMINIMAL_STACK_SIZE, NULL, GTGPS_DECK_TASK_PRI, NULL);
