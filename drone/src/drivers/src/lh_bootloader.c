...
bool lhblFlashWriteFW(uint8_t* data, uint32_t length) {
    ...
    for (pageCount = 0; pageCount < nbrPages && status; pageCount++) {
        status = lhblFlashWritePage(LH_FW_ADDR + pageCount * LH_FLASH_PAGE_SIZE, LH_FLASH_PAGE_SIZE, &data[pageCount * LH_FLASH_PAGE_SIZE]);
        lhblFlashWaitComplete();

        lhblFlashRead(LH_FW_ADDR + pageCount * LH_FLASH_PAGE_SIZE, LH_FLASH_PAGE_SIZE, flashReadBuf);
    ...
    if (lastPageSize && status) {
        status = lhblFlashWritePage(LH_FW_ADDR + pageCount * LH_FLASH_PAGE_SIZE, lastPageSize, &data[pageCount * LH_FLASH_PAGE_SIZE]);
        lhblFlashWaitComplete();

        lhblFlashRead(LH_FW_ADDR + pageCount * LH_FLASH_PAGE_SIZE, lastPageSize, flashReadBuf);
