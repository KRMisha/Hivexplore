...
#define FIX_TEMP \
    25 // Fixed Temperature. ASL is a function of pressure and temperature, but as the temperature changes so much (blow a little towards
       // the flie and watch it drop 5 degrees) it corrupts the ASL estimates. TLDR: Adjusting for temp changes does more harm than good.
...
bool ms5611ReadPROM() {
    ...
    for (i = 0; i < MS5611_PROM_REG_COUNT; i++) {
        // start read sequence
        status = i2cdevWriteByte(I2Cx, devAddr, I2CDEV_NO_MEM_ADDR, MS5611_PROM_BASE_ADDR + (i * MS5611_PROM_REG_SIZE));
        // Read conversion
        if (status) {
