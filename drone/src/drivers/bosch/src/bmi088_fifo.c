 ...
 * @return None
 */
static void unpack_accel_frm(struct bmi088_sensor_data* accel, uint16_t* idx, uint16_t* accel_idx, uint8_t frm,
...
static uint16_t unpack_gyro_frm(struct bmi088_sensor_data* data, uint16_t* idx, uint16_t* gyro_idx, uint8_t frm,
    ...
    if ((frm == BMI088_GYRO_ALL_INT_DATA) || (frm == BMI088_GYRO_X_DATA) || (frm == BMI088_GYRO_Y_DATA) || (frm == BMI088_GYRO_Z_DATA)) {
        /* Partial read, then skip the data */
