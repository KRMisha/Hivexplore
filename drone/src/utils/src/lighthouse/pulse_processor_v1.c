            ...
            if (stateV1->sweeps[sensor].state == sweepStorageStateValid) {
                ...
                if (delta < FRAME_LENGTH) {
                    ...
                    *axis = stateV1->currentAxis;
                    ...
                    pulseProcessorBaseStationMeasuremnt_t* bsMeasurement =
                        &result->sensorMeasurementsLh1[sensor].baseStatonMeasurements[stateV1->currentBaseStation];
...
static void decodeAndApplyBaseStationCalibrationData(pulseProcessor_t* state) {
    if (!state->bsCalibration[0].valid && ootxDecoderProcessBit(&state->ootxDecoder[0], getOotxDataBit(state->v1.currentSync0Width))) {
        printBSInfo(&state->ootxDecoder[0].frame);
        lighthouseCalibrationInitFromFrame(&state->bsCalibration[0], &state->ootxDecoder[0].frame);
    }
    if (!state->bsCalibration[1].valid && ootxDecoderProcessBit(&state->ootxDecoder[1], getOotxDataBit(state->v1.currentSync1Width))) {
        printBSInfo(&state->ootxDecoder[1].frame);
        lighthouseCalibrationInitFromFrame(&state->bsCalibration[1], &state->ootxDecoder[1].frame);
    }
}

static bool processSync(pulseProcessor_t* state, unsigned int timestamp, unsigned int width, pulseProcessorResult_t* angles,
                        int* baseStation, int* axis) {
    bool anglesMeasured = false;
...
static bool processWhenSynchronized(pulseProcessor_t* state, int sensor, unsigned int timestamp, unsigned int width,
                                    pulseProcessorResult_t* angles, int* baseStation, int* axis) {
    bool anglesMeasured = false;
...
bool pulseProcessorV1ProcessPulse(pulseProcessor_t* state, const pulseProcessorFrame_t* frameData, pulseProcessorResult_t* angles,
                                  int* baseStation, int* axis) {
    bool anglesMeasured = false;
    ...
    if (!state->v1.synchronized) {
        ...
        anglesMeasured =
            processWhenSynchronized(state, frameData->sensor, frameData->timestamp, frameData->width, angles, baseStation, axis);
...
TESTABLE_STATIC int findSyncTime(const pulseProcessorPulse_t pulseHistory[], uint32_t* sync0Time) {
    ...
    if (nFoundSync0 == 2) {
        ...
        bool isFrameLengthWithinBoundaries =
            (frameLength < (FRAME_LENGTH + MAX_FRAME_LENGTH_NOISE) && frameLength > (FRAME_LENGTH - MAX_FRAME_LENGTH_NOISE));
...
void pulseProcessorV1ProcessValidAngles(pulseProcessorResult_t* angles, int basestation) {
    ...
    for (int sensor = 0; sensor != PULSE_PROCESSOR_N_SENSORS; sensor++) {
        if (angles->sensorMeasurementsLh1[sensor].baseStatonMeasurements[basestation].validCount != 0) {
            ...
            validAngles |=
                sensorBits << (sensor * PULSE_PROCESSOR_N_SWEEPS + basestation * PULSE_PROCESSOR_N_SWEEPS * PULSE_PROCESSOR_N_SENSORS);
        }
    }
}

uint8_t pulseProcessorV1AnglesQuality() {
    return __builtin_popcount(validAngles) * 1.0 /
           (PULSE_PROCESSOR_N_SWEEPS * PULSE_PROCESSOR_N_SENSORS * PULSE_PROCESSOR_N_BASE_STATIONS) * 255;
}
