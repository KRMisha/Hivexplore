...
static void sensorsDeviceInit(void) {
    ...
    if (rslt == BSTDR_OK) {
        ...
        rslt |= bmi160_get_sensor_data(BMI160_GYRO_ONLY, NULL, &gyr, &bmi160Dev);
        struct bmi160_sensor_data acc;
        rslt |= bmi160_get_sensor_data(BMI160_ACCEL_ONLY, &acc, NULL, &bmi160Dev);
    } else {
...
static void sensorsGyroGet(Axis3i16* dataOut, uint8_t device) {
    ...
    case SENSORS_BMI160:
        ...
        break;
    case SENSORS_BMI055:
        bmi055_get_gyro_data((struct bmi055_sensor_data*)dataOut, &bmi055Dev);
        break;
    }
}
...
static void sensorsAccelGet(Axis3i16* dataOut, uint8_t device) {
    ...
    case SENSORS_BMI160:
        ...
        break;
    case SENSORS_BMI055:
        bmi055_get_accel_data((struct bmi055_sensor_data*)dataOut, &bmi055Dev);
        /* scale to 16 bit */
...
static void sensorsTask(void* param) {
    ...
    while (1) {
        ...
        if (!allSensorsAreCalibrated) {
            ...
            case SENSORS_BMI160:
                sensorsApplyBiasAndScale(&sensors.gyro, &gyroPrim, &bmi160GyroBias.value, SENSORS_BMI160_DEG_PER_LSB_CFG);
                break;
            case SENSORS_BMI055:
                sensorsApplyBiasAndScale(&sensors.gyro, &gyroPrim, &bmi055GyroBias.value, SENSORS_BMI055_DEG_PER_LSB_CFG);
                break;
            }

            sensorsAccIIRLPFilter(&accelPrim, &accelPrimLPF, &accelPrimStoredFilterValues, (int32_t)sensorsAccLpfAttFactor);

            switch (accelPrimInUse) {
            case SENSORS_BMI160:
                sensorsApplyBiasAndScale(&accelPrimScaled, &accelPrimLPF, &bmi160AccelBias.value, SENSORS_BMI160_G_PER_LSB_CFG);
                break;
            case SENSORS_BMI055:
                sensorsApplyBiasAndScale(&accelPrimScaled, &accelPrimLPF, &bmi055AccelBias.value, SENSORS_BMI055_G_PER_LSB_CFG);
                break;
            }
            ...
            switch (gyroSecInUse) {
            case SENSORS_BMI160:
                sensorsApplyBiasAndScale(&sensors.gyroSec, &gyroSec, &bmi160GyroBias.value, SENSORS_BMI160_DEG_PER_LSB_CFG);
                break;
            case SENSORS_BMI055:
                sensorsApplyBiasAndScale(&sensors.gyroSec, &gyroSec, &bmi055GyroBias.value, SENSORS_BMI055_DEG_PER_LSB_CFG);
                break;
            }

            sensorsAccIIRLPFilter(&accelSec, &accelSecLPF, &accelSecStoredFilterValues, (int32_t)sensorsAccLpfAttFactor);

            switch (accelSecInUse) {
            case SENSORS_BMI160:
                sensorsApplyBiasAndScale(&accelSecScaled, &accelSecLPF, &bmi160AccelBias.value, SENSORS_BMI160_G_PER_LSB_CFG);
                break;
            case SENSORS_BMI055:
                sensorsApplyBiasAndScale(&accelSecScaled, &accelSecLPF, &bmi055AccelBias.value, SENSORS_BMI055_G_PER_LSB_CFG);
                break;
            }
...
static void sensorsBiasMalloc(BiasObj* bias) {
    ...
    bias->bufStart = pvPortMalloc(SENSORS_NBR_OF_BIAS_SAMPLES * sizeof(Axis3i16));
