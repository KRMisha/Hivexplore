...
static void sensorsTask(void* param) {
    ...
    while (1) {
        if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY)) {
            ...
            uint8_t dataLen = (uint8_t)(SENSORS_MPU6500_BUFF_LEN + (isMagnetometerPresent ? SENSORS_MAG_BUFF_LEN : 0) +
            ...
            if (isBarometerPresent) {
                processBarometerMeasurements(
                    &(buffer[isMagnetometerPresent ? SENSORS_MPU6500_BUFF_LEN + SENSORS_MAG_BUFF_LEN : SENSORS_MPU6500_BUFF_LEN]));
            }
...
static bool processAccScale(int16_t ax, int16_t ay, int16_t az) {
    ...
    if (!accBiasFound) {
        accScaleSum +=
            sqrtf(powf(ax * SENSORS_G_PER_LSB_CFG, 2) + powf(ay * SENSORS_G_PER_LSB_CFG, 2) + powf(az * SENSORS_G_PER_LSB_CFG, 2));
        accScaleSumCount++;
...
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, MS5611,
          &isBarometerPresent) // TODO: Rename MS5611 to LPS25H. Client needs to be updated at the same time.
...
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, MS5611,
          &isLPS25HTestPassed) // TODO: Rename MS5611 to LPS25H. Client needs to be updated at the same time.
