...
static void oaInit() {
    ...
    pca95x4ConfigOutput(~(OA_PIN_UP | OA_PIN_RIGHT | OA_PIN_LEFT | OA_PIN_FRONT | OA_PIN_BACK));

    pca95x4ClearOutput(OA_PIN_UP | OA_PIN_RIGHT | OA_PIN_LEFT | OA_PIN_FRONT | OA_PIN_BACK);

    isInit = true;

    xTaskCreate(oaTask, OA_DECK_TASK_NAME, 2 * configMINIMAL_STACK_SIZE, NULL, OA_DECK_TASK_PRI, NULL);
}
