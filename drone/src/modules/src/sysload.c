...
static void timerHandler(xTimerHandle timer) {
    if (triggerDump != 0) {
        ...
        // CPU usage is since last dump in % compared to total time spent in tasks. Note that time spent in interrupts will be included in
        // measured time. Stack usage is displayed as nr of unused bytes at peak stack usage.
