...
bool mspIsRequestValid(MspObject* pMspObject) {
    if (pMspObject->requestHeader.preamble[0] != MSP_PREAMBLE_0 || pMspObject->requestHeader.preamble[1] != MSP_PREAMBLE_1) {
        // Invalid preamble
        DEBUG_PRINT("MSP Request has invalid preamble %d %d\n",
                    pMspObject->requestHeader.preamble[0],
                    pMspObject->requestHeader.preamble[1]);
        return false;
    }
    ...
    if (pMspObject->requestCrc != mspComputeCrc((uint8_t*)&pMspObject->requestHeader, sizeof(pMspObject->requestHeader))) {
        // CRC does not match
        DEBUG_PRINT("MSP Request has invalid crc (%d != %d)\n",
                    pMspObject->requestCrc,
