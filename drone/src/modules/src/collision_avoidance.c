...
//
static struct vec sidestepGoal(collision_avoidance_params_t const* params, struct vec goal, bool modifyIfInside, float const A[],
                               float const B[], float projectionWorkspace[], int nRows) {
...
void collisionAvoidanceUpdateSetpointCore(collision_avoidance_params_t const* params, collision_avoidance_state_t* collisionState,
                                          int nOthers, float const* otherPositions, float* workspace, setpoint_t* setpoint,
                                          sensorData_t const* sensorData, state_t const* state) {
    //
    ...
    if (setpoint->mode.x == modeVelocity) {
        ...
        if (vinpolytope(vzero(), A, B, nRows, inPolytopeTolerance)) {
            ...
            struct vec nearestInCell = vprojectpolytope(vzero(),
                                                        A,
                                                        B,
                                                        projectionWorkspace,
                                                        nRows,
        ...
        struct vec const setPosRelativeNew = sidestepGoal(params, setPosRelative, false, A, B, projectionWorkspace, nRows);
...
void collisionAvoidanceUpdateSetpoint(setpoint_t* setpoint, sensorData_t const* sensorData, state_t const* state, uint32_t tick) {
