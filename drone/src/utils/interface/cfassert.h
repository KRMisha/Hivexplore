...
#define ASSERT_DMA_SAFE(PTR) \
    if (((uint32_t)(PTR) >= 0x10000000) && ((uint32_t)(PTR) <= 0x1000FFFF)) \
 ...
 * gdb> info line *0x<PC>
 */
void storeAssertHardfaultData(unsigned int r0, unsigned int r1, unsigned int r2, unsigned int r3, unsigned int r12, unsigned int lr,
                              unsigned int pc, unsigned int psr);
