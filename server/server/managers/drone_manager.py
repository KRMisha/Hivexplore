...
class DroneManager(ABC):
    def __init__(self, web_socket_server: WebSocketServer, logger: Logger, map_generator: MapGenerator):
        ...
        self._web_socket_server.bind(SocketEvent.MissionState, self._set_mission_state)
        self._web_socket_server.bind(SocketEvent.SetLed, self._set_led_enabled)
    ...
    def _send_drone_ids(self, client_id=None):
        if client_id is None:
            self._web_socket_server.send_message(SocketEvent.DroneIds, self._get_drone_ids())
        else:
            self._web_socket_server.send_message_to_client(client_id, SocketEvent.DroneIds, self._get_drone_ids())
    ...
    def _log_velocity_callback(self, drone_id: str, data: Dict[str, float]):
        ...
        self._web_socket_server.send_drone_message(SocketEvent.Velocity, drone_id, round(velocity_magnitude, 3))
    ...
    def _log_drone_status_callback(self, drone_id: str, data: Dict[str, int]):
        ...
        self._web_socket_server.send_drone_message(SocketEvent.DroneStatus, drone_id, drone_status.name)
    ...
    def _web_socket_connect_callback(self, client_id: str):
        ...
        self._web_socket_server.send_message_to_client(client_id, SocketEvent.MissionState, self._mission_state.name)

        for drone_id, is_led_enabled in self._drone_leds.items():
            self._web_socket_server.send_drone_message_to_client(client_id, SocketEvent.SetLed, drone_id, is_led_enabled)
    ...
    def _set_mission_state(self, mission_state_str: str):
        ...
        self._web_socket_server.send_message(SocketEvent.MissionState, mission_state_str)
    ...
    def _set_led_enabled(self, drone_id: str, is_enabled: bool):
        if self._is_drone_id_valid(drone_id):
            ...
            self._web_socket_server.send_drone_message(SocketEvent.SetLed, drone_id, is_enabled)
