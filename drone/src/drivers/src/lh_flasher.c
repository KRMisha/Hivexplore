                                      ...
                                      ".global incbin_" STR( \
                                          name) "_end\n" \
                                                ...
                                                "incbin_" STR(name) "_end:\n" \
                                                                    ".byte 0\n" \
                                                                    ".align 4\n" STR(name) "Size:\n" \
                                                                                           ".int incbin_" STR(name) "_end - incbin_" STR( \
                                                                                               name) "_start\n"); \
...
static bool flashWriteFW(uint8_t* data, uint32_t length) {
    ...
    for (pageCount = 0; pageCount < nbrPages && status; pageCount++) {
        status = flashWritePage(pageCount * LH_FLASH_PAGE_SIZE, LH_FLASH_PAGE_SIZE, &data[pageCount * LH_FLASH_PAGE_SIZE]);
        flashWaitComplete();

        flashRead(pageCount * LH_FLASH_PAGE_SIZE, LH_FLASH_PAGE_SIZE, flashReadBuf);
    ...
    if (lastPageSize && status) {
        status = flashWritePage(pageCount * LH_FLASH_PAGE_SIZE, lastPageSize, &data[pageCount * LH_FLASH_PAGE_SIZE]);
        flashWaitComplete();

        flashRead(pageCount * LH_FLASH_PAGE_SIZE, lastPageSize, flashReadBuf);
