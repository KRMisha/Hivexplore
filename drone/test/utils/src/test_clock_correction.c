...
void testGetClockCorrection() {
    ...
    clockCorrectionStorage_t clockCorrectionStorage = {.clockCorrection = clockCorrection, .clockCorrectionBucket = 0};
...
void testCalculateClockCorrectionWithValidInputDataWithWrapAround() {
    ...
    // This second part of the test veryfies that when the bit mask has a higher number of bits than the timestamps, the result is wrong if
    // wrap around happens
    const uint64_t wrongMask = 0x1FFFFFFFFFF; // 41 bits
    const double result2 =
        clockCorrectionEngineCalculate(new_t_in_cl_reference, old_t_in_cl_reference, new_t_in_cl_x, old_t_in_cl_x, wrongMask);
...
void testUpdateClockCorrectionWithSampleInTheOuterLimitOfTheSpecs() {
    ...
    clockCorrectionStorage_t clockCorrectionStorage = {.clockCorrection = clockCorrection, .clockCorrectionBucket = clockCorrectionBucket};
...
void testUpdateClockCorrectionWithSampleInTheInnerLimitOfTheAcceptableNoise() {
    ...
    const unsigned int clockCorrectionBucket = 2;
    const double clockCorrectionCandidate =
        nextafter(clockCorrection + CLOCK_CORRECTION_ACCEPTED_NOISE, 1); // First value in the acceptable noise
    ...
    // Assert
    const double expectedClockCorrection =
        clockCorrection * CLOCK_CORRECTION_FILTER + clockCorrectionCandidate * (1.0 - CLOCK_CORRECTION_FILTER);
    const unsigned int expectedClockCorrectionBucket = clockCorrectionBucket + 1;
