...
static uint32_t handleLedringmemGetSize(void) {
    return sizeof(ledringmem);
}
...
static uint32_t handleTimingmemGetSize(void) {
    return sizeof(ledringtimingsmem);
}
...
#define COPY_COLOR(dest, orig) \
    dest[0] = orig[0]; \
    dest[1] = orig[1]; \
    dest[2] = orig[2]
#define ADD_COLOR(dest, o1, o2) \
    dest[0] = (o1[0] >> 1) + (o2[0] >> 1); \
    dest[1] = (o1[1] >> 1) + (o2[1] >> 1); \
    dest[2] = (o1[2] >> 1) + (o2[2] >> 1);
#define LIMIT(a) ((a > 255) ? 255 : (a < 0) ? 0 : a)
#define SIGN(a) ((a >= 0) ? 1 : -1)
#define DEADBAND(a, b) ((a < b) ? 0 : a)
#define LINSCALE(domain_low, domain_high, codomain_low, codomain_high, value) \
    ((codomain_high - codomain_low) / (domain_high - domain_low)) * (value - domain_low) + codomain_low
#define SET_WHITE(dest, intensity) \
    dest[0] = intensity; \
    dest[1] = intensity; \
    dest[2] = intensity;
...
/**************** White spin ***************/
#if NBR_LEDS > 12
static const uint8_t whiteRing[NBR_LEDS][3] = {
    {32, 32, 32},
    {8, 8, 8},
    {2, 2, 2},
    BLACK,
...
#if NBR_LEDS > 12
static const uint8_t blueRing[NBR_LEDS][3] = {
    {64, 64, 255},
    {32, 32, 64},
    {8, 8, 16},
    BLACK,
...
#if NBR_LEDS > 12
static const uint8_t colorRing[NBR_LEDS][3] = {
    {0, 0, 32},
    {0, 0, 16},
    {0, 0, 8},
    {0, 0, 4},
...
static bool isTimeMemDone(ledtiming current) {
    return current.duration == 0 && current.color[0] == 0 && current.color[1] == 0;
}
    ...
    for (int i = 0; i < NBR_LEDS; i++)
        for (int j = 0; j < 3; j++)
            outputBuffer[(i + shift) % NBR_LEDS][j] =
                percentShift * currentBuffer[i][j] + (1 - percentShift) * currentBuffer[(i + 1) % NBR_LEDS][j];
}

/**************** Effect list ***************/

Ledring12Effect effectsFct[] = {
    blackEffect,      whiteSpinEffect,  colorSpinEffect, tiltEffect,          brightnessEffect, spinEffect2,
    doubleSpinEffect, solidColorEffect, ledTestEffect,   batteryChargeEffect, boatEffect,       siren,
    gravityLight,     virtualMemEffect, fadeColorEffect, rssiEffect,          locSrvStatus,     timeMemEffect,
    lighthouseEffect,
};
...
static void ledring12Init(DeckInfo* info) {
    ...
    timer = xTimerCreate("ringTimer", M2T(50), pdTRUE, NULL, ledring12Timer);
