 ...
 * @param skip The number of inital evaluations to ignore failures for. This is a convenient way to let the system warm up a bit before
 * reporting problems.
 */
void rateSupervisorInit(rateSupervisor_t* context, const uint32_t osTimeMs, const uint32_t evaluationIntervalMs, const uint32_t minCount,
                        const uint32_t maxCount, const uint8_t skip);
