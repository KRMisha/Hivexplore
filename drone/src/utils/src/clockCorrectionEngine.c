...
/**
 Logging all the clock correction information requires scaling the values repeatedly, which is computer intense. Thus, the logging
 functionality is enabled at compile time with the CLOCK_CORRECTION_ENABLE_LOGGING flag.
 */
...
/**
 Obtains the clock correction from a clockCorrectionStorage_t object. This is the recommended public API to obtain the clock correction,
 instead of getting it directly from the storage object.
 */
...
/**
 Truncates a timestamp to the number of bits of the mask. This truncation ensures that the value returned is a valid time event, even if the
...
/**
 ...
 @param mask A mask as long as the number of bits used to represent the timestamps. Used to calculate a valid timestamp, even if wrapped
 arounds of the time counter happened at some point
 @return The necessary clock correction to apply to timestamps measured by clock x, in order to obtain their value like if the measurement
 was done by the reference clock. Or -1 if it was not possible to perform the computation. Example: timestamp_in_cl_reference =
 clockCorrection * timestamp_in_cl_x
 */
double clockCorrectionEngineCalculate(const uint64_t new_t_in_cl_reference, const uint64_t old_t_in_cl_reference,
                                      const uint64_t new_t_in_cl_x, const uint64_t old_t_in_cl_x, const uint64_t mask) {
...
/**
 Updates the clock correction only if the provided value follows certain conditions. This is used to discard wrong clock correction
 measurements.
 @return True if the provided clock correction sample ir reliable, false otherwise. A sample is reliable when it is in the accepted noise
 level (which means that we already have two or more samples that are similar) and has been LP filtered.
 */
bool clockCorrectionEngineUpdate(clockCorrectionStorage_t* storage, const double clockCorrectionCandidate) {
    ...
    if (-CLOCK_CORRECTION_ACCEPTED_NOISE < difference && difference < CLOCK_CORRECTION_ACCEPTED_NOISE) {
        ...
        const double newClockCorrection =
            currentClockCorrection * CLOCK_CORRECTION_FILTER + clockCorrectionCandidate * (1.0 - CLOCK_CORRECTION_FILTER);
        ...
        if (shouldAcceptANewClockReference) {
            if (CLOCK_CORRECTION_SPEC_MIN < clockCorrectionCandidate && clockCorrectionCandidate < CLOCK_CORRECTION_SPEC_MAX) {
                // We do not fill the bucket and accept the clock correction sample as reliable: a sample is reliable when it is in the
                // accepted noise level (which means that we already have two or more samples that are similar) and has been LP filtered.
                // See: https://github.com/bitcraze/crazyflie-firmware/pull/328
